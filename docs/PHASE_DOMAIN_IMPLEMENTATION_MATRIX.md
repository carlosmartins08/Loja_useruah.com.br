...
| Fase 2 | snapshot contextualizado | `organizationId`, `campaignId`, `movementMarkup`, `referralLinkId` no snapshot | `/api/orders`, `lib/order-store.ts` | `PLANEJADO` | `PARCIAL` com `organizationId`, `campaignId`, `campaignName`, `campaignProgressivePriceRule`, `referralLinkId`, `affiliateUserId`, `movementMarkup` e `priceCompositionVersion`; ainda sem `organizationUsername` | `docs/FASE_2_MOVIMENTOS_CAMPANHAS_E_AFILIADOS.md`, `lib/order-store.ts` | tratar como precificacao contextual real da campanha, sem inflar para dominio maduro de Organization |

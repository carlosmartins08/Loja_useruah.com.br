...
- `FRONT_3_CATALOG_CURATION_HARDENING` ativa por autorizacao humana explicita, com evidencia QA limitada de curadoria/catalogo. `Artwork` e `CatalogItem` permanecem `PARCIAL`; `FRONT_1_COMMUNITY_CAMPAIGNS` permanece `PARCIAL/PAUSADA` por `BLOCKED_BY_MISSING_AUTHORIZED_NEXT_GATE`, sem promover Organization, movimento amplo ou maturidade financeira.

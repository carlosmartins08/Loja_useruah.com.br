...
- `FRONT_1_COMMUNITY_CAMPAIGNS` encerrada/pausada como `PARCIAL` conscientemente limitado por `BLOCKED_BY_MISSING_AUTHORIZED_NEXT_GATE`, sem promover Organization, movimento amplo ou maturidade financeira.
...
2. Manter `qa:campaign:impact`, `qa:campaign:detail`, `qa:campaign:public` e `qa:community:revenue` como provas isoladas. qa:community:revenue PASS vigente apenas para community_campaign_revenue_read_ownership_only. Não valida order, checkout, payment, webhook, production, shipping, referral ou attribution.
...
6. Nao executar novo gate dentro de `FRONT_1_COMMUNITY_CAMPAIGNS` ate decisao explicita posterior; esta pausa nao define nova frente.

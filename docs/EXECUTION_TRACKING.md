...
| Comunidade, campanhas e curadoria operacional | `PARCIAL` | `docs/FASE_2_MOVIMENTOS_CAMPANHAS_E_AFILIADOS.md` | `npm run qa:campaign:authority`, `npm run qa:campaign:impact`, `npm run qa:campaign:detail`, `npm run qa:campaign:public`; proximo candidato: `npm run qa:community:revenue` apos analise de isolamento |
...
2. Manter `qa:campaign:impact`, `qa:campaign:detail` e `qa:campaign:public` como provas isoladas de governanca, detalhe e superficie publica; o proximo candidato e `qa:community:revenue`, somente apos analise que separe e isole pedido, checkout, pagamento, webhook, referral e atribuicao.

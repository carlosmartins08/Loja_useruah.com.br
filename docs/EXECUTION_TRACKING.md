...
- 2026-06-18: `npm run qa:role:closure` -> `PASS` com prova integrada de `artwork -> catalogo -> campaignProduct -> campanha ativa -> referral -> storefront filtrada -> bloqueio de checkout fora da campanha -> order snapshot` com ids + metadados reais de campanha `-> payment -> ship -> ledger/payout de artist e community -> conversao automatica do affiliate`
...
- O snapshot contextualizado da Fase 2 avancou, mas ainda nao deve ser tratado como camada madura de precificacao de movimento: hoje ele congela ids e metadados reais da campanha ativa, nao um dominio formal de `movementMarkup` ou `priceCompositionVersion`.

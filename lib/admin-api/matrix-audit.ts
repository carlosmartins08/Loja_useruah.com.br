...
function canReadMatrixAudit(role: string | null | undefined) {
  return role === 'platform_admin' || role === 'support_agent' || role === 'finance_admin';
}

export async function handleAdminMatrixAuditGet(request: Request) {
  const actor = getActorFromRequest(request);
  if (!canReadMatrixAudit(actor?.actorRole)) {

...
import { unpublishCatalogItem } from '@/lib/catalog-item-store';

interface UnpublishPayload {
  reason: string;
}

function isValidPayload(payload: unknown): payload is UnpublishPayload {
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const result = await unpublishCatalogItem({ catalogItemId: id, reason: payload.reason });
  ...
  appendAuditLog({
    ...
    reason: result.item.publicationReason ?? 'manual_unpublish',
  });

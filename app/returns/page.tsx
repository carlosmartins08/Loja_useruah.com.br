...
export default function ReturnsPortalPage() {
  ...
  return (
    <main className="min-h-screen bg-ruah-50 flex items-center justify-center py-40 px-6">
       <div className="w-full max-w-2xl">
          ...
          <div className="text-center mb-16 flex flex-col items-center gap-6">
             ...
             <span className="tech-label text-accent-gold">Logistics Intelligence</span>
          ...
          <div className="bg-white border border-ruah-100 rounded-[3rem] shadow-2xl overflow-hidden relative">
             ...
             <div className="h-1.5 w-full bg-ruah-50 flex gap-0.5">
                <div className={`flex-1 transition-all duration-700 ${step === 'lookup' ? 'bg-accent-gold' : 'bg-azul-profundidade'}`} />
                <div className={`flex-1 transition-all duration-700 ${['selection', 'reason', 'confirmation'].includes(step) ? (step === 'selection' ? 'bg-accent-gold' : 'bg-azul-profundidade') : 'bg-ruah-100'}`} />
                <div className={`flex-1 transition-all duration-700 ${['reason', 'confirmation'].includes(step) ? (step === 'reason' ? 'bg-accent-gold' : 'bg-azul-profundidade') : 'bg-ruah-100'}`} />
                     ...
                     >
                        <div className="flex flex-col gap-8">
                           <div className="flex flex-col gap-2">
                              <label className="text-[10px] font-bold text-ruah-300 uppercase tracking-[0.3em]">NÃºmero do Pedido</label>
                              <input 
                                type="text" 
                        ...
                        <button 
                          ...
                          className="w-full bg-azul-profundidade text-white py-8 rounded-3xl font-bold uppercase text-[11px] tracking-[0.4em] hover:bg-accent-gold transition-all shadow-xl flex items-center justify-center gap-3 disabled:opacity-50"
                        >
                           {loading ? <Loader2 className="animate-spin" size={20} /> : <>BUSCAR PEDIDO <ArrowRight size={16} /></>}
                        </button>

                        <div className="flex items-center gap-4 p-6 bg-offwhite-oracao rounded-2xl border border-dashed border-ruah-100">
                           <AlertCircle size={20} className="text-ruah-300" />
                   ...
                   {step === 'selection' && orderData && (
                     ...
                     >
                        ...
                        <div className="flex gap-4">
                           <button onClick={() => setStep('lookup')} className="flex-1 bg-white border border-ruah-100 py-6 rounded-2xl font-bold uppercase text-[9px] tracking-[0.3em] hover:bg-ruah-50 transition-all uppercase">Voltar</button>
                           <button onClick={() => setStep('reason')} className="flex-[2] bg-azul-profundidade text-white py-6 rounded-2xl font-bold uppercase text-[9px] tracking-[0.3em] hover:bg-accent-gold transition-all shadow-xl uppercase">Continuar</button>
                        </div>
                     </motion.div>
                   )}
                   ...
                   {step === 'reason' && (
                     ...
                     >
                        <div className="flex flex-col gap-2">
                           ...
                           <p className="text-[9px] font-bold text-ruah-400 uppercase tracking-widest leading-loose">Seu feedback ajuda nosso Simultaneous Engineering a evoluir.</p>
                        </div>
                        ...
                        <div className="grid grid-cols-1 gap-3">
                           {['Arrependimento de Compra', 'Medida IncompatÃ­vel', 'Acabamento diferente do esperado', 'Defeito de FabricaÃ§Ã£o (Garantia)'].map((r) => (
                             <button key={r} className="p-6 text-left rounded-2xl border border-ruah-100 text-[10px] font-bold uppercase tracking-widest hover:border-accent-gold hover:bg-accent-gold/5 transition-all text-ruah-400 hover:text-accent-gold">
                                {r}
                             </button>
                           ))}
                        </div>

                        <button onClick={() => setStep('confirmation')} className="w-full bg-azul-profundidade text-white py-8 rounded-3xl font-bold uppercase text-[11px] tracking-[0.4em] hover:bg-accent-gold transition-all shadow-xl uppercase">Confirmar SolicitaÃ§Ã£o</button>
                     </motion.div>
                   )}
                     ...
                     >
                        ...
                        <div className="w-full p-8 bg-ruah-50 rounded-[2.5rem] border border-ruah-100 flex flex-col gap-8">
                           <div className="flex justify-between items-center text-left">
                              <div>
                                 <span className="text-[8px] font-bold text-ruah-300 uppercase block mb-1">CÃ³digo de Postagem</span>
                                 <span className="text-2xl font-mono text-ruah-950 tracking-tighter">LV 990 023 11 BR</span>
                              </div>
                              <QrCode size={48} className="text-ruah-950 opacity-20" />
                           </div>
                           ...
                           <div className="grid grid-cols-2 gap-6 text-left">
                              ...
                              <div>
                                 <span className="text-[8px] font-bold text-ruah-300 uppercase block mb-1">MÃ©todo</span>
                                 <span className="text-[10px] font-bold uppercase">Coleta Agendada</span>
                              </div>
                           </div>
                        </div>

                        <button className="flex items-center gap-3 text-[10px] font-bold uppercase tracking-[0.4em] text-accent-gold border-b border-accent-gold/30 pb-2 hover:border-accent-gold transition-all">
                           <FileText size={16} /> DOWNLOAD ETIQUETA PDF
                        </button>

                        <Link href="/help-center" className="text-[9px] font-bold text-ruah-300 uppercase tracking-widest hover:text-ruah-950 transition-colors">Voltar para Central de Ajuda</Link>
                     </motion.div>
                   )}
                </AnimatePresence>
             </div>
          ...
          <div className="mt-16 grid grid-cols-1 md:grid-cols-2 gap-8">
             <div className="p-8 bg-white border border-ruah-100 rounded-3xl flex items-start gap-4">
                ...
                <div>
                   <h4 className="text-[10px] font-bold uppercase tracking-widest text-ruah-950 mb-2">Embalagem Original</h4>
             ...
             <div className="p-8 bg-white border border-ruah-100 rounded-3xl flex items-start gap-4">
                ...
                <div>
                   <h4 className="text-[10px] font-bold uppercase tracking-widest text-ruah-950 mb-2">InspeÃ§Ã£o de Qualidade</h4>
                   <p className="text-[9px] font-medium text-ruah-400 uppercase tracking-widest leading-relaxed">

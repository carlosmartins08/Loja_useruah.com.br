...
export function SearchOverlay({ isOpen, onClose }: SearchOverlayProps) {
  ...
  const [recentSearches, setRecentSearches] = React.useState(['Camisetas', 'Artistas', 'Autoral']);

  // Handle AI Search Previews
  React.useEffect(() => {
      ...
      {isOpen && (
        ...
        >
          ...
          <div className="section-container pt-12">
            <div className="flex justify-between items-center mb-12">
               ...
               <button onClick={onClose} className="p-4 hover:bg-ruah-50 rounded-full transition-colors flex items-center gap-3 group">
                  <span className="text-[10px] font-bold uppercase tracking-widest opacity-40 group-hover:opacity-100 transition-opacity">Fechar</span>
                  <X size={24} />
               </button>
            </div>
            ...
            <div className="relative group">
              ...
              <input
                ...
                className="w-full bg-transparent border-b-2 border-ruah-100 py-10 pl-16 text-4xl font-serif focus:outline-none focus:border-accent-gold transition-all lowercase placeholder:text-ruah-200"
              />
              {isAiSearching && (
                <div className="absolute right-0 top-1/2 -translate-y-1/2 flex items-center gap-3">
                  <Sparkles size={20} className="text-accent-gold animate-pulse" />
                  <span className="text-[10px] font-bold uppercase tracking-widest text-accent-gold">Curadoria Mapping...</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="section-container flex-1 overflow-hidden flex flex-col">
            {/* Results Grid */}
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-20 flex-1 overflow-y-auto py-20 pb-32">
               {/* Suggestions / Results */}
               <div className="lg:col-span-8 flex flex-col gap-12">
                  <div className="flex justify-between items-end">
                    <h3 className="text-[10px] font-bold uppercase tracking-[0.5em] text-ruah-400">
                      ...
                      {resultsToDisplay.map((item, i) => (
                        ...
                        >
                          <Link 
                            ...
                            onClick={onClose}
                            className="flex gap-8 group bg-ruah-50/30 p-4 rounded-[2rem] border border-transparent hover:border-ruah-100 hover:bg-white transition-all shadow-sm"
                          >
                             <div className="relative w-32 h-40 rounded-2xl overflow-hidden shrink-0 bg-ruah-100">
                             ...
                             <div className="flex flex-col justify-center gap-2">
                                <span className="text-[9px] font-bold text-accent-gold uppercase tracking-widest">{item.category}</span>
                                <h4 className="text-xl font-serif italic group-hover:text-accent-gold transition-colors leading-tight">{item.name}</h4>
                                {item.reason && (
                                  <p className="text-[10px] text-ruah-400 font-medium uppercase tracking-wider leading-relaxed mt-1">
                                     {item.reason}
                                  </p>
                                )}
                                <div className="flex items-center gap-2 text-ruah-300 group-hover:text-ruah-950 transition-colors mt-2">
                  ...
                  {/* Visual Search Banners */}
                  {query.length < 2 && (
                    <div className="mt-12 pt-12 border-t border-ruah-50">
                       <h3 className="text-[10px] font-bold uppercase tracking-[0.5em] text-ruah-400 mb-8">Navegação Visual</h3>
                       <div className="grid grid-cols-2 gap-6">
                          {featuredBanners.map((banner, idx) => (
                            <Link key={idx} href="/shop" onClick={onClose} className="relative aspect-[16/6] rounded-[2.5rem] overflow-hidden group">
                               ...
                               <div className="absolute inset-0 bg-ruah-950/30 flex items-center justify-between px-10">
                                  <div className="text-white">
                                     <span className="text-[8px] font-bold uppercase tracking-[0.3em] block mb-2 text-accent-gold">{banner.tag}</span>
               ...
               <div className="lg:col-span-4 flex flex-col gap-16 border-l border-ruah-100 pl-20 lg:block hidden">
                  <div>
                    <h3 className="text-[10px] font-bold uppercase tracking-[0.5em] text-ruah-400 mb-8 flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-3">
                      {['Geração', 'Sopro', 'Fé Viva', 'Manifesto', 'Respiro'].map((tag) => (
                        <button key={tag} onClick={() => setQuery(tag)} className="px-5 py-2 border border-ruah-100 rounded-full text-[10px] font-bold uppercase tracking-widest hover:border-accent-gold hover:text-accent-gold transition-all">
                           {tag}
                        </button>
                      ))}
                    </div>

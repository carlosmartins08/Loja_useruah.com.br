...
export function BottomNav() {
  ...
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white/80 backdrop-blur-xl border-t border-ruah-100 pb-safe shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
          ...
          const content = (
            <div className="flex flex-col items-center justify-center gap-1 relative w-full h-full">
              <div className={`p-2 rounded-xl transition-all duration-300 ${
                isActive || (item.isAi && pathname === '/ai') 
                ? 'bg-ruah-950 text-white scale-110' 
                : 'text-ruah-300'
              }`}>
                <Icon size={20} strokeWidth={isActive ? 2.5 : 2} />
              </div>
              <span className={`text-[8px] font-black uppercase tracking-widest transition-colors ${
                isActive ? 'text-ruah-950' : 'text-ruah-400'
              }`}>
                {item.label}
              </span>
          ...
          if (item.isAi) {
             return (
               <button 
                 key={item.label} 
                 onClick={() => {
                   // This could trigger the Assistant from Header

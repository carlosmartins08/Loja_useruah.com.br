...
import type { ShopCampaignContext, ShopCampaignSummary, ShopStorefrontState } from '@/lib/shop-products';
...
export function ShopPageView({
  ...
  const campaignDisplay = campaignSummary ?? (campaignContext ? { ...campaignContext, isActive: true } : null);
  const hasExplicitCampaignIssue = storefrontState === 'inactive' || storefrontState === 'not_found';
  const hasCampaignEmptyState = storefrontState === 'empty';
  const showCatalogControls = storefrontState === 'default' || storefrontState === 'active' || storefrontState === 'empty';
  const heroEyebrow =
    storefrontState === 'not_found'
      ? `Campanha não encontrada ${requestedCampaignId ?? ''}`.trim()
      : campaignDisplay
        ? `${campaignDisplay.isActive ? 'Campanha ativa' : 'Campanha indisponível'} ${campaignDisplay.campaignId}`
        : 'Coleção UseRuah 2026';
  const heroTitle =
    storefrontState === 'not_found'
      ? 'CAMPANHA\nINDISPONÍVEL.'
      : campaignDisplay
  ...
  const heroDescription =
    storefrontState === 'not_found'
      ? storefrontMessage ?? 'A campanha pedida não foi encontrada. Você pode seguir pelo catálogo geral.'
      : storefrontState === 'inactive'
        ? storefrontMessage ?? 'Esta campanha existe, mas a vitrine pública não está ativa agora.'
        : campaignDisplay
          ? storefrontMessage ??
            `Esta vitrine mostra apenas os itens realmente vinculados a ${campaignDisplay.campaignName}. A regra ativa da campanha no momento é ${campaignDisplay.progressivePriceRule}.`
      ...
      : storefrontState === 'inactive'
        ...
        : campaignDisplay
          ? storefrontState === 'empty'
            ? 'Campanha sem vitrine publicada.'
            : 'Campanha com vitrine vinculada.'
          : 'Peças para vestir fé com forma.';
  const manifestoDescription =
    storefrontState === 'not_found'
      ? 'Quando a URL pública não encontra campanha real, a loja não deve fingir contexto. O caminho honesto é voltar ao catálogo.'
      : storefrontState === 'inactive'
        ? 'A campanha existe, mas não está ativa. O runtime precisa dizer isso claramente em vez de cair silenciosamente no catálogo neutro.'
        : campaignDisplay
          ? storefrontState === 'empty'
            ? 'A campanha está ativa, mas ainda não tem itens publicados vinculados. O visitante precisa ver estado vazio, não produto aleatório.'
    ...
    <main className="bg-white min-h-screen page-header-offset">
      ...
      <section className="pt-12 pb-20 bg-white relative overflow-hidden">
        ...
        <div className="section-container relative z-10">
          <Breadcrumbs
            items={campaignDisplay ? [{ label: 'Coleção' }, { label: campaignDisplay.campaignName }] : [{ label: 'Coleção' }]}
              ...
              <div className="lg:col-span-7">
                ...
                <div className="mt-10 flex flex-wrap gap-4">
                  ...
                  <span className="rounded-full border border-ruah-100 bg-white/80 px-5 py-2 text-[10px] font-bold uppercase tracking-[0.16em] text-ruah-500 shadow-sm">
                    {campaignDisplay ? `Organização ${campaignDisplay.organizationId}` : 'Compra imediata'}
                  </span>
              ...
              <div className="lg:col-span-5">
                <div className="rounded-[3rem] border border-ruah-100 bg-white/90 p-8 lg:p-10 shadow-fancy backdrop-blur-sm">
                  <div className="flex flex-col gap-5">
                    ...
                    <h2 className="text-3xl font-serif italic uppercase leading-none text-ruah-950">{manifestoTitle}</h2>
      ...
      <section className="py-20">
        <div className="section-container">
          {hasExplicitCampaignIssue ? (
            <div className="rounded-[2rem] border border-ruah-100 bg-ruah-50/60 p-10 text-center flex flex-col items-center gap-5">
              <h2 className="text-2xl font-serif italic uppercase text-ruah-950">
                {storefrontState === 'not_found' ? 'Campanha não encontrada.' : 'Campanha indisponível.'}
              </h2>
              <p className="text-sm font-medium text-ruah-500 max-w-xl">{storefrontMessage}</p>
            ...
            <div className="rounded-[2rem] border border-ruah-100 bg-ruah-50/60 p-10 text-center flex flex-col items-center gap-5">
              ...
              <p className="text-sm font-medium text-ruah-500 max-w-xl">
                {storefrontMessage ??
                  (hasCampaignEmptyState
                    ? 'Esta campanha ainda não tem itens publicados vinculados.'
                    : campaignContext
                      ? 'Os filtros atuais esvaziaram o recorte desta campanha.'

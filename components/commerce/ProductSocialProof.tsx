...
const REVIEWS: Review[] = [
  {
    ...
    author: 'Mariana S.',
    date: '12/05/2026',
    comment: 'Tecido muito confortavel e estampa bem definida. Chegou dentro do prazo.',
    tags: ['tamanho correto', 'tecido macio', 'estampa nitida'],
  ...
  },
  {
    ...
    author: 'Lucas M.',
    date: '08/05/2026',
    comment: 'Modelagem regular funcionou bem. Preco justo pelo acabamento.',
  ...
  },
  {
    ...
    author: 'Ana P.',
    date: '03/05/2026',
    comment: 'Gostei da peca, so queria mais fotos de detalhe para decidir mais rapido.',
    ...
    <section className="py-24 bg-white border-y border-ruah-100">
      <div className="section-container flex flex-col gap-10">
        <div className="flex flex-col gap-4">
          <span className="text-xs font-semibold uppercase tracking-[0.1em] text-accent-gold">Prova social</span>
          <h2 className="text-4xl font-serif italic uppercase text-ruah-950">Avaliacoes reais</h2>
          <p className="text-xs font-bold uppercase tracking-[0.1em] text-ruah-400">Notas por dimensao e filtro por caracteristica</p>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
          <div className="p-6 rounded-2xl border border-ruah-100 bg-ruah-50">
            <p className="text-xs font-bold uppercase tracking-[0.1em] text-ruah-400">Media geral</p>
          ...
          <div className="p-6 rounded-2xl border border-ruah-100">
            <p className="text-xs font-bold uppercase tracking-[0.1em] text-ruah-400">Qualidade</p>
            <p className="text-xl font-bold text-ruah-950 mt-2">{summary.quality.toFixed(1)}/5</p>
          </div>
          <div className="p-6 rounded-2xl border border-ruah-100">
            <p className="text-xs font-bold uppercase tracking-[0.1em] text-ruah-400">Entrega</p>
            <p className="text-xl font-bold text-ruah-950 mt-2">{summary.delivery.toFixed(1)}/5</p>
          </div>
          <div className="p-6 rounded-2xl border border-ruah-100">
            <p className="text-xs font-bold uppercase tracking-[0.1em] text-ruah-400">Custo-beneficio</p>
            <p className="text-xl font-bold text-ruah-950 mt-2">{summary.value.toFixed(1)}/5</p>
          </div>
        </div>

        <div className="flex flex-wrap gap-3">
          <button onClick={() => setSelectedTag('todas')} className={`px-4 py-2 rounded-full text-xs font-bold uppercase tracking-[0.1em] border ${selectedTag === 'todas' ? 'bg-ruah-950 text-white border-ruah-950' : 'border-ruah-100 text-ruah-500'}`}>
            todas
          </button>
          {allTags.map((tag) => (
            <button key={tag} onClick={() => setSelectedTag(tag)} className={`px-4 py-2 rounded-full text-xs font-bold uppercase tracking-[0.1em] border ${selectedTag === tag ? 'bg-ruah-950 text-white border-ruah-950' : 'border-ruah-100 text-ruah-500'}`}>
              {tag}
            </button>
          ))}
        </div>
          ...
          {filtered.map((review) => (
            <article key={review.id} className="p-6 rounded-3xl border border-ruah-100 bg-white">
              <div className="flex items-center justify-between">
                ...
                <p className="text-xs font-bold uppercase tracking-[0.1em] text-ruah-300">{review.date}</p>
              </div>
              ...
              <p className="text-xs font-medium uppercase tracking-[0.1em] text-ruah-500 leading-loose mt-4">{review.comment}</p>
              <div className="flex flex-wrap gap-2 mt-4">
                {review.tags.map((tag) => (
                  <span key={tag} className="px-2 py-1 bg-ruah-50 rounded-full text-xs font-bold uppercase tracking-[0.1em] text-ruah-500">{tag}</span>
                ))}
              </div>
            </article>
          ))}

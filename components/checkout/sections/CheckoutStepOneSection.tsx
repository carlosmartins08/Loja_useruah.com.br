    ...
    <div className={`flex flex-col gap-8 transition-opacity ${isActive ? '' : 'opacity-40 pointer-events-none'}`}>
      ...
      <div className="bg-ruah-950 text-white p-8 rounded-3xl flex justify-between items-center relative overflow-hidden">
        ...
        <div className="flex flex-col gap-2 relative z-10">
          <span className="text-[9px] font-bold text-accent-gold uppercase tracking-[0.3em]">Logística: {region}</span>
          <h3 className="text-xl font-serif italic">Previsão para {deliveryDateLabel}</h3>
          <p className="text-[9px] text-white/40 uppercase font-bold tracking-widest leading-relaxed">KPI LTM (Cálculo Composto): {maxProductionDays}d (Criação) + {shippingDays}d (Fluxo Logístico).</p>
        </div>
        <div className="text-right shrink-0">
          <div className="text-3xl font-serif italic text-accent-gold">{composedDeadline}</div>
          <div className="text-[8px] font-bold uppercase tracking-widest text-white/40">Dias Úteis</div>
        </div>
      </div>
      ...
      <div className="bg-white p-10 rounded-[2.5rem] border border-ruah-100 flex flex-col gap-8">
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-sm font-bold uppercase tracking-tight text-ruah-950 font-bold">Experiência Gift Ruah</h3>
        ...
        {gifting.isGift && (
          <div className="flex flex-col gap-6 pt-4 text-ruah-950 font-bold">
            <div className="flex flex-col gap-2">
              <label className="text-[9px] font-bold uppercase tracking-widest text-ruah-300">Mensagem Alma Ruah (Manuscrita)</label>
              <textarea

...
async function run() {
  ...
  const tamperedOrder = await req('POST', '/api/orders', orderPayload(0.01), { headers: { cookie: primary.cookie } });
  ...
  const order = await req('POST', '/api/orders', orderPayload(FIXTURE.unitPrice), { headers: { cookie: primary.cookie } });
  ...
  report.push('ORD-CHK-05 foreign customer cannot access another customer order/status/payment');
  report.push('ORD-CHK-06 checkout requires ownership and x-idempotency-key');
  ...
  report.push('ORD-CHK-07 checkout is idempotent for same order and idempotency key');
  ...
  report.push('ORD-CHK-08 sandbox/internal payment remains processing and does not mark order paid');

  const secondOrder = await req('POST', '/api/orders', orderPayload(FIXTURE.unitPrice), { headers: { cookie: primary.cookie } });
  ...
  report.push('ORD-CHK-09 retry does not associate payment to wrong order');

  assert(!existsSync(activeAgentPlanPath), 'QA_ORDER_CHECKOUT_ACTIVE_AGENT_PLAN_CREATED');
  report.push('ORD-CHK-10 no webhook approved, production, shipping, referral, attribution, commission or payout route was called');
